...
WorkspaceModel::WorkspaceModel(QObject *parent) : QObject(parent) {
    ...
    m_backingFileBlockCountLimit = mappingBlockLimit;

    // Reuse initialize storage routine here
    clearStorage();
...
void WorkspaceModel::clearStorage() {
...
uint8_t *WorkspaceModel::blockFromSequenceNumber(uint64_t blockSeqNumber) {
    return (m_storageMapping + blockSeqNumber * m_blockSize);
}

uint64_t WorkspaceModel::sequenceNumberFromBlock(uint8_t *block) {
    return (block - m_storageMapping) / m_blockSize;
}

bool WorkspaceModel::growToBlocksLong(uint64_t blockCount) {
...
Result<uint64_t, WorkspaceModel::Error> WorkspaceModel::allocateBlock() {
    // Sanity check
...
void WorkspaceModel::freeBlock(uint64_t blockSeqNumber) {
...
void WorkspaceModel::printFreeList() {
