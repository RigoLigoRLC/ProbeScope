...
Session::Session(QObject *parent)
    : QObject(parent), _shellProcess(nullptr), _emulation(nullptr), _monitorActivity(false), _monitorSilence(false),
      _notifiedActivity(false), _autoClose(true), _wantedClose(false), _silenceSeconds(10), _isTitleChanged(false),
...
bool Session::isRunning() const {
    ...
    qDebug() << __FUNCTION__ << ": UNIMPLEMENTED";
    return true;
}
...
void Session::run() {
     ...
     * So i added a check to see if /bin/bash exists - if no then we use $SHELL - if that does not exist either, we fall
     * back to /bin/sh As far as i know /bin/sh exists on every unix system.. You could also just put some ifdef
     * __FREEBSD__ here but i think these 2 filechecks are worth their computing time on any system - especially with
     * the problem on arch linux being there too.
     */
...
int Session::processId() const {
    return 0; // static_cast<int>(_shellProcess->processId());
}
